<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" errorPage="product_error.jsp"%>
<!DOCTYPE HTML>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>SO:edit</title>

<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/bootstrap-3.3.2-dist/css/bootstrap.min.css">

</head>
<body>


<div class="page-header">
<h1>ServcieOP <small>제품 변경</small></h1>
</div>

<a href="product_list.do" class="btn btn-default btn-lg">제품 리스트로</a>
<a href="/suptman" class="btn btn-default btn-lg">홈으로</a>


<div style="margin:0 auto; width: 500px;">

<form method=post action="edit_product.do" id="editFrm">
  <div class="form-group">
    <input type="hidden" name="pIndex" id="pIndex" value="${pVO.pIndex}">
    <label class="control-label" for="pVendor">Vendor*</label>
    <input type="text" class="form-control" name="pVendor" id="pVendor" value="${pVO.pVendor}" placeholder="Vendor">
  </div>
  <div class="form-group">
    <label for="pName" class="control-label">Name*</label>
    <input type="text" class="form-control" name="pName" id="pName" value="${pVO.pName}" placeholder="Name">
  </div>
  <div class="form-group">
    <label for="pVersion" class="control-label">Version*</label>
    <input type="text" class="form-control" name="pVersion" id="pVersion" value="${pVO.pVersion}" placeholder="Version">
  </div> 
  <div class="form-group">
    <input type="button" class="btn btn-default btn-lg btn-block" value="변경하기" onclick="chkInp();"/>  
  </div>
</form>
</div>


<div class="page-header">
</div>
<h1>ServcieOP <small>제품 변경</small></h1>


<!-- jQuery (부트스트랩의 자바스크립트 플러그인을 위해 필요합니다) -->
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
<!-- 모든 컴파일된 플러그인을 포함합니다 (아래), 원하지 않는다면 필요한 각각의 파일을 포함하세요 -->
<script src="${pageContext.request.contextPath}/resources/bootstrap-3.3.2-dist/js/bootstrap.min.js"></script>

<script>
function chkInp(){
	
	var a=$("#pVendor").val().replace(/ /g,"").length
	var b=$("#pName").val().replace(/ /g,"").length
	var c=$("#pVersion").val().replace(/ /g,"").length
	if((a*b*c)==0){
		alert("별표시는 필수 입력사항 입니다.")
	}else{
		$('#editFrm').submit();
	}
}

</script>


</body>
</html>